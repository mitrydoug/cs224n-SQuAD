...
if __name__ == '__main__':
    ...
    POWERS=np.arange(0.0, 0.21, 0.05)
    F1={}
    EM={}
    best_f1 = 0.0
    best_em = 0.0
    for p in POWERS:
        os.system(('python code/main.py --ans_len_dist_power={} '
                   '--preatt_hidden_size=150 '
                   '--postatt_start_hidden_size=150 '
                   '--postatt_end_hidden_size=100 '
                   '--glove_set=large '
                   '--context_len=300 '
                   '--mode=official_eval '
                   '--json_in_path=data/dev-v1.1.json '
                   '--ckpt_load_dir=experiments/character_embeddings')
                   .format(p))
    ...
    with open('em-1.json', 'w') as f:
        json.dump(EM, f)
    POWERS=np.arange(best_f1-0.04, best_f1+0.05, 0.01)
    for p in POWERS:
        if p in F1: continue
        os.system(('python code/main.py --ans_len_dist_power={} '
                   '--mode=official_eval '
    ...
    with open('em-2.json', 'w') as f:
        json.dump(EM, f)
    POWERS=np.arange(best_em-0.04, best_em+0.05, 0.01)
    for p in POWERS:
        if p in EM: continue
        os.system(('python code/main.py --ans_len_dist_power={} '
                   '--mode=official_eval '
